...
function CardFace({ cardKey, large }: { cardKey: string; large?: boolean }) {
  ...
  return (
    <div style={{ width: size, height: Math.round(size * 1.4), borderRadius: large ? 12 : 6, background: PANEL, border: `${large ? 2 : 1}px solid ${ACCENT}`, boxShadow: large ? `0 0 32px ${ACCENT}30` : "none", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", flexShrink: 0 }}>
...
function CardLookSection() {
  ...
  function CardImagePlaceholder({ label }: { label: string }) {
    return (
      <div style={{ width: "100%", aspectRatio: "5/7", borderRadius: 14, border: `1px dashed ${BORDER}`, background: "#0e0d0b", display: "flex", flexDirection: "column" as const, alignItems: "center", justifyContent: "center", gap: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid #2a241b`, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <rect x="1" y="3" width="14" height="10" rx="2" stroke="#3a342b" strokeWidth="1.2" />
            <circle cx="5.5" cy="7" r="1.5" stroke="#3a342b" strokeWidth="1.2" />
            <path d="M1 11l4-3 3 2.5 3-4 4 4.5" stroke="#3a342b" strokeWidth="1.2" strokeLinejoin="round" />
          </svg>
        </div>
        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#3a342b", textAlign: "center" as const, letterSpacing: "0.1em" }}>
  ...
  return (
    <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "24px 26px", marginBottom: 20 }}>
      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.18em", color: "#6f6759", textTransform: "uppercase" as const, marginBottom: 6 }}>Card design</div>
      <h3 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 24, fontWeight: 400, margin: "0 0 5px" }}>What the card looks like</h3>
      <p style={{ fontSize: 13, color: "#847b6d", margin: "0 0 22px", lineHeight: 1.6 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.14em", color: "#5f594f", marginBottom: 10, textTransform: "uppercase" as const }}>Front — your artwork</div>
          {!frontErr ? (
            <img
              src="/card-front.png"
              alt="Card front"
        ...
        <div>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.14em", color: "#5f594f", marginBottom: 10, textTransform: "uppercase" as const }}>Back — shared template</div>
          {!backErr ? (
            <img
              src="/card-back.png"
              alt="Card back"
      ...
      <div style={{ marginTop: 16, fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#4a4538", lineHeight: 1.6 }}>
...
function ArtworkPreviewSection() {
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (artworkImg.current) {
      ...
      const placeholderInk = hasTemplate ? "#b0a996" : "#2e2820";
      const placeholderBg = hasTemplate ? "#f3efe6" : "#111009";
  ...
  return (
    <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "24px 26px", marginTop: 24 }}>
      ...
      <div style={{ display: "flex", gap: 32, alignItems: "flex-start", flexWrap: "wrap" as const }}>
        <div style={{ flexShrink: 0 }}>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.12em", color: "#5f594f", textAlign: "center" as const, marginBottom: 4 }}>← 2.5 in / 750 px →</div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, color: "#5f594f", writingMode: "vertical-rl" as const, transform: "rotate(180deg)", letterSpacing: "0.1em" }}>← 3.5 in / 1050 px →</div>
            <div>
              <canvas ref={canvasRef} width={CW} height={CH} style={{ width: CARD_DISPLAY_W, height: CARD_DISPLAY_H, borderRadius: 8, display: "block", background: "#1a1814" }} />
              {artworkUrl && (
                <button onClick={handleDownload} style={{ marginTop: 10, width: "100%", height: 34, borderRadius: 8, border: "1px solid #2c271f", background: "transparent", color: "#c9bfaf", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          ...
          >
            ...
            <div style={{ fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 13.5, color: "#c9bfaf", marginBottom: 5 }}>{artworkUrl ? "Click or drag to replace" : "Click or drag artwork here"}</div>
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#5f594f" }}>PNG or JPG — 750 × 1050 px at 300 dpi (standard playing card)</div>
            <input ref={fileInputRef} type="file" accept="image/*" hidden onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
          </div>
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: "block", fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.14em", color: "#5f594f", textTransform: "uppercase" as const, marginBottom: 8 }}>
              Your name / handle
            </label>
            <input
              type="text"
              ...
              style={{ width: "100%", height: 38, borderRadius: 8, border: "1px solid #2e2820", background: "#1a1814", color: "#efe9dd", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 13.5, padding: "0 12px", boxSizing: "border-box" as const }}
            />
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#5f594f", marginTop: 6 }}>Printed in the bottom-left margin, as shown on the reference card front.</div>
          </div>
  ...
  if (result.status === "conflict") {
    return (
      <div style={{ background: PANEL, border: "1px solid #262119", borderRadius: 16, padding: "24px 26px", marginBottom: 24 }}>
        <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12, color: "#d09a6a", margin: "0 0 14px" }}>A conflict occurred — two people submitted at the same time. Please try again.</p>
      ...
      <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
        ...
        <div style={{ flex: 1 }}>
          {isNew ? (
            <>
              ...
              <p style={{ color: "#9a9286", margin: "0 0 20px", fontSize: 14, lineHeight: 1.55 }}>
                Your card is locked in. Create artwork sized 1.8 × 2.8 in and submit it using the form below.
              </p>
            </>
          ) : (
            <>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" as const }}>
            <a href={formUrl} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", height: 44, padding: "0 22px", borderRadius: 10, background: ACCENT, color: "#1a1408", fontWeight: 700, fontSize: 15, textDecoration: "none", fontFamily: "'Hanken Grotesk', sans-serif" }}>
              Submit your artwork →
            </a>
            <a href={WHATSAPP_GROUP_URL} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", height: 44, padding: "0 22px", borderRadius: 10, background: "transparent", border: "1px solid #25d36660", color: "#25d366", fontWeight: 700, fontSize: 15, textDecoration: "none", fontFamily: "'Hanken Grotesk', sans-serif" }}>
              Join the WhatsApp group →
            </a>
          </div>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: "#5f594f", marginTop: 12 }}>
...
function VolumeTwoPanel({ nextProject }: { nextProject: Project | null }) {
  return (
    <div style={{ background: PANEL, border: "1px solid #262119", borderRadius: 16, padding: "32px 26px", marginBottom: 24, textAlign: "center" as const }}>
      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, letterSpacing: "0.2em", color: "#6f6759", textTransform: "uppercase" as const, marginBottom: 14 }}>All 54 cards claimed</div>
          ...
          {[
            ...
            <div key={n} style={{ display: "flex", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, opacity: step === n ? 1 : 0.4 }}>
                <div style={{ width: 22, height: 22, borderRadius: "50%", border: `1.5px solid ${step === n ? ACCENT : "#3a342b"}`, background: step > n ? ACCENT : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: step > n ? "#1a1408" : (step === n ? ACCENT : "#3a342b"), flexShrink: 0 }}>
                  {step > n ? "✓" : n}
                </div>
                <span style={{ color: step === n ? "#c9bfaf" : "#4a4538", letterSpacing: "0.1em" }}>{label.toUpperCase()}</span>
              </div>
              {i === 0 && <div style={{ width: 40, height: 1, background: step > 1 ? ACCENT : "#2a241b", margin: "0 12px", flexShrink: 0 }} />}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ marginBottom: 36 }}>
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, letterSpacing: "0.2em", color: "#5f594f", textTransform: "uppercase" as const, marginBottom: 10 }}>The Holding · Project 001</div>
              <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 52, fontWeight: 400, margin: "0 0 16px", lineHeight: 1 }}>54 Hands</h1>
              <p style={{ fontSize: 15.5, color: "#9a9286", lineHeight: 1.65, margin: "0 0 20px", maxWidth: 580 }}>
              ...
              <div style={{ display: "flex", gap: 20, fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: "#6f6759", flexWrap: "wrap" as const }}>
                {[
                  ...
                  <div key={label}>
                    <span style={{ color: "#3a342b" }}>{label}: </span>
                    <span style={{ color: "#c9bfaf" }}>{value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "22px 26px", marginBottom: 20 }}>
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.18em", color: "#6f6759", textTransform: "uppercase" as const, marginBottom: 18 }}>How it works</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
              <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "22px 22px" }}>
                ...
                <div style={{ marginBottom: 18 }}>
                  {[
                    { label: "Artists (net revenue, equal split)", pct: "100%", color: ACCENT, bar: ACCENT },
              ...
              <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "22px 22px", display: "flex", flexDirection: "column" as const }}>
                ...
                <div style={{ marginTop: 16, height: 6, borderRadius: 999, background: "#1e1b15", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: "0%", background: ACCENT, borderRadius: 999 }} />
                </div>
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#4a4538", marginTop: 6 }}>0 / {MILESTONE} sold — milestone in progress</div>
              </div>
            </div>

            {/* Production costs */}
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "24px 26px", marginBottom: 20 }}>
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.18em", color: "#6f6759", textTransform: "uppercase" as const, marginBottom: 6 }}>Production costs</div>
              <h3 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 24, fontWeight: 400, margin: "0 0 5px" }}>How production is funded</h3>
              <p style={{ fontSize: 13, color: "#847b6d", margin: "0 0 20px", lineHeight: 1.65 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                {[
                  ...
                  <div key={item} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", borderRadius: 10, background: "#0e0d0b", border: "1px solid #1e1b15" }}>
                    <div>
                      <div style={{ fontSize: 13, color: "#c9bfaf", fontWeight: 500, marginBottom: 3 }}>{item}</div>
                      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#4a4538" }}>{note}</div>
                    </div>
                    <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12, color: "#5f594f", flexShrink: 0, marginLeft: 12 }}>{value}</div>
                  </div>
                ))}
              </div>
              <div style={{ padding: "12px 14px", borderRadius: 9, background: "rgba(216,162,74,0.06)", border: `1px solid ${ACCENT}25`, fontSize: 13, color: "#9a9286", lineHeight: 1.65 }}>
            ...
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "28px 30px", marginBottom: 20 }}>
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.2em", color: "#6f6759", textTransform: "uppercase" as const, marginBottom: 6 }}>54 Hands: The First Gathering</div>
              ...
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 24 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                    {[
                      ...
                      <div key={term} style={{ background: "#0e0d0b", border: "1px solid #1e1b15", borderRadius: 10, padding: "14px 16px" }}>
                        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: ACCENT, marginBottom: 6 }}>{term}</div>
                ...
                <div style={{ background: "rgba(216,162,74,0.06)", border: `1px solid ${ACCENT}25`, borderRadius: 10, padding: "16px 18px", fontSize: 13.5, color: "#9a9286", lineHeight: 1.7 }}>
            ...
            <div style={{ border: `1px solid ${ACCENT}30`, borderRadius: 14, overflow: "hidden", marginBottom: 20 }}>
              <div style={{ background: `linear-gradient(135deg, rgba(216,162,74,0.12) 0%, rgba(216,162,74,0.04) 100%)`, borderBottom: `1px solid ${ACCENT}20`, padding: "22px 26px" }}>
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, letterSpacing: "0.2em", color: ACCENT + "aa", textTransform: "uppercase" as const, marginBottom: 8 }}>For participating artists</div>
              ...
              <div style={{ background: PANEL, padding: "24px 26px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
                  {[
                    ...
                    <div key={title} style={{ display: "flex", gap: 14, padding: "16px 18px", borderRadius: 10, background: "#0e0d0b", border: "1px solid #1e1b15" }}>
                      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 16, color: ACCENT, flexShrink: 0, lineHeight: 1, marginTop: 2 }}>→</div>
                      <div>
                ...
                <div style={{ borderTop: "1px solid #1e1b15", paddingTop: 20, display: "flex", gap: 20, alignItems: "center" }}>
                  <div style={{ flex: 1, fontSize: 14.5, color: "#c9bfaf", lineHeight: 1.7 }}>
            ...
            {/* AI disclosure */}
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "20px 24px", display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 32 }}>
            ...
            {/* Checkbox + continue */}
            <div style={{ background: PANEL, border: `1.5px solid ${BORDER}`, borderRadius: 16, padding: "28px 30px" }}>
              <h3 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 24, fontWeight: 400, margin: "0 0 6px" }}>Ready to participate?</h3>
              <p style={{ fontSize: 13.5, color: "#847b6d", margin: "0 0 22px", lineHeight: 1.55 }}>
              ...
              <label style={{ display: "flex", alignItems: "flex-start", gap: 14, cursor: "pointer", marginBottom: 24 }}>
                <div
                  ...
                  style={{ width: 20, height: 20, borderRadius: 6, border: `1.5px solid ${agreed ? ACCENT : "#3a342b"}`, background: agreed ? ACCENT : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1, transition: "all 0.15s", cursor: "pointer" }}
                >
                  {agreed && (
                    <svg width="11" height="9" viewBox="0 0 11 9" fill="none">
                      <path d="M1 4l3.5 3.5L10 1" stroke="#1a1408" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  )}
                </div>
                <span style={{ fontSize: 14, color: agreed ? "#c9bfaf" : "#847b6d", lineHeight: 1.6, transition: "color 0.15s", userSelect: "none" as const }}>
              ...
              <button
                ...
                style={{ height: 50, padding: "0 32px", borderRadius: 12, border: "none", background: agreed ? ACCENT : "#2a241b", color: agreed ? "#1a1408" : "#4a4538", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 15, fontWeight: 700, cursor: agreed ? "pointer" : "not-allowed", transition: "all 0.2s" }}
              >
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
              <div>
                ...
                <p style={{ fontSize: 14, color: "#847b6d", margin: 0 }}>
                  {takenCount} of {totalSlots} cards claimed — {totalSlots - takenCount} remaining
                </p>
              </div>
              <button
                onClick={() => { setStep(1); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                style={{ height: 38, padding: "0 16px", borderRadius: 9, border: "1px solid #2c271f", background: "transparent", color: "#c9bfaf", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}
              >
                ← Back to details
              </button>
            </div>
            ...
            {/* Progress bar */}
            <div style={{ height: 3, background: "#1e1b15", borderRadius: 999, marginBottom: 24, overflow: "hidden" }}>
            ...
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "20px 22px", marginBottom: 24 }}>
                  ...
                  <div key={suit} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div style={{ flex: 1, display: "flex", gap: 4 }}>
                      {VALUES.map((value) => {
                        ...
                        return (
                          <div key={key} id={`card-${key}`} title={claimant ?? "Available"} style={{ flex: 1, aspectRatio: "1/1.4", borderRadius: 5, border: isHighlighted ? `1px solid ${ACCENT}` : claimant ? "1px solid #221e17" : "1px solid #2a2519", background: isHighlighted ? `rgba(216,162,74,0.14)` : claimant ? "#111009" : "rgba(216,162,74,0.03)", display: "flex", flexDirection: "column" as const, alignItems: "center", justifyContent: "center", gap: 1, minWidth: 0, transition: "border-color 0.15s", boxShadow: isHighlighted ? `0 0 12px ${ACCENT}20` : "none" }}>
                            ...
                            {claimant ? (
                              <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: 7, color: isHighlighted ? ACCENT : "#3a342b", lineHeight: 1, overflow: "hidden", maxWidth: "90%", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{claimant.split(" ")[0]}</div>
                            ) : (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    {(["Joker Red", "Joker Black"] as const).map((joker) => {
                      ...
                      return (
                        <div key={joker} id={`card-${joker}`} title={claimant ?? joker} style={{ width: 46, aspectRatio: "1/1.4", borderRadius: 5, border: isHighlighted ? `1px solid ${ACCENT}` : claimant ? "1px solid #221e17" : "1px solid #2a2519", background: isHighlighted ? "rgba(216,162,74,0.14)" : claimant ? "#111009" : "rgba(216,162,74,0.03)", display: "flex", flexDirection: "column" as const, alignItems: "center", justifyContent: "center", gap: 1, boxShadow: isHighlighted ? `0 0 12px ${ACCENT}20` : "none" }}>
                          ...
                          {claimant && <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: 7, color: isHighlighted ? ACCENT : "#3a342b", lineHeight: 1 }}>{claimant.split(" ")[0]}</div>}
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ display: "flex", gap: 16, marginTop: 14, fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#6f6759" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: "#111009", border: "1px solid #221e17", display: "inline-block" }} />Claimed</span>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: "rgba(216,162,74,0.03)", border: "1px solid #2a2519", display: "inline-block" }} />Available</span>
                  ...
                  <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column" as const, gap: 14 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      {[
                        ...
                        <div key={label}>
                          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, letterSpacing: "0.14em", color: "#847b6d", textTransform: "uppercase" as const, marginBottom: 7 }}>{label}</div>
                    ...
                    {fieldError && (
                      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: "#e07070", background: "rgba(224,112,112,0.08)", border: "1px solid rgba(224,112,112,0.18)", borderRadius: 8, padding: "10px 12px" }}>{fieldError}</div>
                    )}
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                      <button type="submit" disabled={submitting} style={{ height: 46, padding: "0 28px", borderRadius: 11, border: "none", background: submitting ? "#6b5a32" : ACCENT, color: "#1a1408", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 15, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer" }}>
                        {submitting ? "Assigning…" : "Claim my card →"}
                      </button>
                      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10.5, color: "#5f594f", lineHeight: 1.5 }}>Assigned randomly.<br />Locked once confirmed.</span>
                    </div>
                  </form>
                </div>
              )
            ...
            {!result && !isFull && (
              <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "20px 26px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20 }}>
                  ...
                  <a href={formUrl} target="_blank" rel="noopener noreferrer" style={{ flexShrink: 0, height: 42, padding: "0 20px", borderRadius: 10, background: ACCENT, color: "#1a1408", fontWeight: 600, fontSize: 14, textDecoration: "none", fontFamily: "'Hanken Grotesk', sans-serif", display: "flex", alignItems: "center", whiteSpace: "nowrap" as const }}>
                    Submit artwork →
                  </a>
                </div>
              </div>
